    ...
    <div className="fixed w-full top-5 flex justify-between max-w-[30rem] rounded-2xl bg-(--muted) border border-(--secondary) px-4 py-2 ">
      ...
      <div>
        {isLoggedIn ? (
          <div className="relative">
            ...
            {show && (
              <div className="absolute top-6 right-0 bg-(--muted) border border-(--secondary) rounded-md px-3 py-2 text-sm text(--text) flex flex-col items-end">
                ...
                <div className="h-[0.5px] w-full bg-(--primary)/10 my-2"></div>
                <button onClick={logout} className="whitespace-nowrap flex gap-2 items-center justify-center text-[#E14434] cursor-pointer">
                  Logout
                  <LogOut size={16} />
                </button>
              </div>
          ...
          <span className="cursor-pointer" onClick={() => openAuthModal()}>Login</span>
        )}
      </div>
    </div>
  );
